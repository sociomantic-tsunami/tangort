...
extern (C) int main(int argc, char **argv)
{
    ...
    // This cleans the value of BP that has been saved on stack upon the entry
    // of this routine. The reason is that D exception handler is using this to
    // go further down the call stack, in order to see if there are any
    // exception handlers.
    //
    // This doesn't work if the glibc is compiled with instructions to omit
    // maintaining frames using BP so this is the last jump that D runtime will
    // be able to perform. The next jump (usually from __libc_start_main to
    // _start)` will be imposible, and the program will halt with SIGSEGV, not
    // telling anything about the exception.
    //
    // Note that D runtime can stop inspecting frames here anyway,
    // because there are no D exceptions handlers in C runtime.
    ...
    bool trapExceptions = false; //rt_trapExceptions;
    ...
    // Restore the BP value on stack that we cleared on this
    // function's prolog
    version (D_InlineAsm_X86)
        asm
        {
