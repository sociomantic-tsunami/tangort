...
//int count = 0;
    while (1)           // for each function on the stack
    {
        ...
        {   // if end of call chain
            ...
            auto o = cast(Object)h;
            assert (o !is null);
            auto e = cast(Exception)o;
            auto name = o.classinfo.name;
            auto msg = o.toString();
            auto file = (e !is null) ? e.file : null;
            if (cast(AssertError)e !is null)
                fprintf(stderr, "assertion failed".ptr);
            else
                fprintf(stderr, "terminated after throwing an uncaught "
                        "instance of '%.*s'".ptr, name.length, name.ptr);
            if (file != "")
                fprintf(stderr, " at %.*s:%zu".ptr, file.length, file.ptr, e.line);
            fprintf(stderr, "\n  toString():  %.*s\n".ptr, msg.length, msg.ptr);
            abort();
            break;
        }
