...
version (linux)
{
    ...
    const TCGETS2 = _IOR!(termios2)('T', 0x2A);
    const TCSETS2 = _IOR!(termios2)('T', 0x2B);
    const TCSETSW2 = _IOW!(termios2)('T', 0x2C);
    const TCSETSF2 = _IOW!(termios2)('T', 0x2D);

    enum { TIOCGRS485 = 0x542E };
    enum { TIOCSRS485 = 0x542F };

    const TIOCGPTN   = _IOR!(uint)('T', 0x30);
    const TIOCSPTLCK = _IOW!(int)('T', 0x31);
    const TIOCGDEV   = _IOR!(uint)('T', 0x32);
