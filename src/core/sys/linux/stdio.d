...
extern(C)
{
    ...
    alias ssize_t function(void *cookie, char *buf, size_t size) cookie_write_function_t;
...
unittest
{
    ...
    extern (C) ssize_t specialRead(void *cookie, char *buf, size_t size)
    {
        memset(buf, 'a', size);
        return size;
    }
    extern (C) int specialClose(void* cookie)
    {
        ++closed;
        return 0;
    }
    extern (C) ssize_t specialWrite(void *cookie, char *buf, size_t size)
    {
