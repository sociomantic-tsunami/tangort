...
// D2 forward compatibility
public alias ThreadException ThreadError;
...
class Thread
{
    ...
    body
    {
        ...
        synchronized( slock )
        {
            mixin("volatile multiThreadedFlag = true;");
            version( Win32 )
            {
    ...
    final Object join( bool rethrow = true )
    {
        ...
        version( Win32 )
        {
            ...
            mixin("volatile m_addr = m_addr.init;");
            CloseHandle( m_hndl );
            m_hndl = m_hndl.init;
        }
        else version( Posix )
    ...
    {
        return _PRIORITY_MIN;
    }

    private static int _PRIORITY_MIN;
    ...
    {
        return _PRIORITY_MAX;
    }

    private static int _PRIORITY_MAX;
    ...
    static this()
    {
        ...
        else version( Posix )
        {
            ...
            _PRIORITY_MIN = sched_get_priority_min( policy );
            assert( PRIORITY_MIN != -1 );

            _PRIORITY_MAX = sched_get_priority_max( policy );
...
private
{
    ...
    version( Posix )
    {
        ...
        version( AsmPPC_Posix ) {} else
        {
            ...
            import core.stdc.posix.ucontext;
...
private
{
    extern (C) void fiber_entryPoint()
    {
        ...
        mixin("volatile Thread.getThis().m_lock = false;");
...
private:
    ...
    body
    {
        ...
        else
        {
            ...
            if ( mprotect(guard, PAGESIZE, PROT_NONE) == -1 )
                throw new FiberException("Unable to set memory protection for stack");
        }
    ...
    body
    {
        // Empty the stack, by setting the stack top equal to the bottom
        m_ctxt.tstack = m_ctxt.bstack;
...
private:
    ...
    final void switchIn()
    {
        ...
        *oldp = getStackTop();
        mixin("volatile tobj.m_lock = true;");
        ...
        tobj.popContext();
        mixin("volatile tobj.m_lock = false;");
