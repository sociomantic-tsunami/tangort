...
void print_options()
{
    ...
    with (options)
    printf("gc.cdgc.opts: verbose=%u, log_file='%s', "
...
{
    if (cstr_eq(opt_name, "verbose".ptr))
        options.verbose = cstdlib.atoi(opt_value);
    else if (cstr_eq(opt_name, "log_file".ptr))
        cstring.strcpy(options.log_file.ptr, opt_value);
    else if (cstr_eq(opt_name, "malloc_stats_file".ptr))
        cstring.strcpy(options.malloc_stats_file.ptr, opt_value);
    else if (cstr_eq(opt_name, "collect_stats_file".ptr))
        cstring.strcpy(options.collect_stats_file.ptr, opt_value);
    else if (cstr_eq(opt_name, "sentinel".ptr))
        options.sentinel = parse_bool(opt_value);
    else if (cstr_eq(opt_name, "mem_stomp".ptr))
        options.mem_stomp = parse_bool(opt_value);
    else if (cstr_eq(opt_name, "conservative".ptr))
        options.conservative = parse_bool(opt_value);
    else if (cstr_eq(opt_name, "fork".ptr))
        options.fork = parse_bool(opt_value);
    else if (cstr_eq(opt_name, "eager_alloc".ptr))
        options.eager_alloc = parse_bool(opt_value);
    else if (cstr_eq(opt_name, "early_collect".ptr))
        options.early_collect = parse_bool(opt_value);
    else if (cstr_eq(opt_name, "min_free".ptr))
        parse_min_free(opt_value);
    else if (cstr_eq(opt_name, "pre_alloc".ptr))
...
package void parse(char* opts_string)
{
    ...
    if (opts_string is null) {
        debug (PRINTF) printf("gc.cdgc.opts: no options overriden\n".ptr);
        return;
    }
...
unittest
{
    ...
    parse("mem_stomp=0:verbose=2:conservative:fork=0:eager_alloc=1".ptr);
    with (options) {
        assert (verbose == 2);
        assert (log_file[0] == '\0');
        assert (sentinel == false);
    ...
    parse("log_file=12345 67890:verbose=1:sentinel=4:mem_stomp=1".ptr);
    with (options) {
        assert (verbose == 1);
    ...
    parse("pre_alloc:min_free=30:early_collect=0".ptr);
    with (options) {
        assert (verbose == 1);
        assert (cstring.strcmp(log_file.ptr, "12345 67890".ptr) == 0);
        assert (sentinel == true);
    ...
    parse("pre_alloc=1:early_collect=0".ptr);
    with (options) {
        assert (verbose == 1);
        assert (cstring.strcmp(log_file.ptr, "12345 67890".ptr) == 0);
        assert (sentinel == true);
    ...
    parse("pre_alloc=5a:min_free=101".ptr);
    with (options) {
        assert (verbose == 1);
        assert (cstring.strcmp(log_file.ptr, "12345 67890".ptr) == 0);
        assert (sentinel == true);
    ...
    parse("pre_alloc=5x:min_free=-1".ptr);
    with (options) {
        assert (verbose == 1);
        assert (cstring.strcmp(log_file.ptr, "12345 67890".ptr) == 0);
        assert (sentinel == true);
    ...
    parse("pre_alloc=09x010:min_free=10a".ptr);
    with (options) {
        assert (verbose == 1);
        assert (cstring.strcmp(log_file.ptr, "12345 67890".ptr) == 0);
        assert (sentinel == true);
    ...
    parse("pre_alloc=5x2:min_free=1.0".ptr);
    with (options) {
        assert (verbose == 1);
        assert (cstring.strcmp(log_file.ptr, "12345 67890".ptr) == 0);
        assert (sentinel == true);
    ...
    parse("pre_alloc=9x5x:min_free=-1".ptr);
    with (options) {
        assert (verbose == 1);
        assert (cstring.strcmp(log_file.ptr, "12345 67890".ptr) == 0);
        assert (sentinel == true);
    ...
    parse("pre_alloc=9x-5:min_free=0".ptr);
    with (options) {
        assert (verbose == 1);
        assert (cstring.strcmp(log_file.ptr, "12345 67890".ptr) == 0);
        assert (sentinel == true);
    ...
    parse("pre_alloc=0x3x0x4:min_free=100".ptr);
    with (options) {
        assert (verbose == 1);
        assert (cstring.strcmp(log_file.ptr, "12345 67890".ptr) == 0);
        assert (sentinel == true);
