...
import gc.cdgc.bits: GCBits;
import gc.cdgc.stats: GCStats, Stats;
import dynarray = gc.cdgc.dynarray;
import os = gc.cdgc.os;
import opts = gc.cdgc.opts;
import gc.cdgc.os: assert_;
...
struct GC
{
    ...
    // indicates if we are reentring the GC code
    // (if > 0 we are running inside the GC context, for example, a finalizer
    // called by the collection called gc_removeRange(), so we don't have to run
    // the invariant or take locks, for example).
    int reentry_level;
...
{
    if (thread_needLock() && gc.reentry_level == 0)
...
bool Invariant()
{
    // Avoid running the invariant in a nested call
    if (gc.reentry_level > 0)
        return true;
    assert_ (gc !is null);
    if (gc.inited) {
        size_t total_mem = 0;
        ...
        for (size_t i = 0; i < gc.pools.length; i++) {
            ...
            pool.Invariant();
            if (i == 0)
                assert_(gc.min_addr == pool.baseAddr);
            if (i + 1 < gc.pools.length)
                assert_(*pool < *gc.pools[i + 1]);
            else if (i + 1 == gc.pools.length)
                assert_(gc.max_addr == pool.topAddr);
        ...
        for (size_t i = 0; i < gc.ranges.length; i++) {
            assert_(gc.ranges[i].pbot);
            assert_(gc.ranges[i].ptop);
            assert_(gc.ranges[i].pbot <= gc.ranges[i].ptop);
        }

        for (size_t i = 0; i < B_PAGE; i++) {
            for (List *list = gc.free_list[i]; list; list = list.next) {
                auto pool = list.pool;
                assert_ (pool !is null);
                auto p = cast(byte*) list;
                assert_ (p >= pool.baseAddr);
                assert_ (p < pool.topAddr);
                assert_ (pool.freebits.test((p - pool.baseAddr) / 16));
                free_mem += binsize[i];
            }
        }
        assert_ (gc.total_mem == total_mem);
        assert_ (gc.free_mem == free_mem);
    }
    return true;
}
...
size_t fullcollect(void *stackTop, bool early = false, bool force_block = false)
{
    ...
    if (opts.options.fork) {
        ...
        assert_ (child_pid != -1); // don't accept errors in non-release mode
        switch (child_pid) {
...
version(none) // BUG: doesn't work because freebits() must also be cleared
{
            ...
            if (bin < B_PAGE)
            {
             ...
             Lnotfree:
                ...
                for (u = 0; u < PAGESIZE; u += size)
                {
                    ...
                    if (pool.freebits.test(bit_i))
                    {
                        assert_ ((p+u) >= pool.baseAddr);
                        assert_ ((p+u) < pool.topAddr);
    ...
    if (gc.collect_end_cb.funcptr) {
        ...
        gc.collect_end_cb(cast(int) (freed + freedpages * PAGESIZE),
                cast(int) ((freedpages + recoveredpages) * PAGESIZE));
    }
...
void initialize()
{
    ...
    opts.parse(cstdlib.getenv("D_GC_OPTS".ptr));
...
private void *malloc(size_t size, uint attrs, size_t* pm_bitmask)
{
    ...
    if (bin < B_PAGE)
    {
        ...
        assert_ ((cast(byte*)list) >= list.pool.baseAddr);
        assert_ ((cast(byte*)list) < list.pool.topAddr);
        gc.free_list[bin] = list.next;
        pool = list.pool;
        bit_i = (p - pool.baseAddr) / 16;
        assert_ (pool.freebits.test(bit_i));
    ...
    if (attrs) {
        ...
        assert_ (bin >= B_PAGE || !pool.freebits.test(bit_i));
    }
...
private void checkNoSync(void *p)
{
    ...
    debug (PTRCHECK)
    {
        ...
        assert_(bin <= B_PAGE);
        size = binsize[bin];
        assert_((cast(size_t)p & (size - 1)) == 0);
...
struct Pool
{
    ...
    void initialize(size_t npages)
    {
        ...
        assert_(poolsize >= POOLSIZE);
        baseAddr = cast(byte *) os.alloc(poolsize);

        // Some of the code depends on page alignment of memory pools
        assert_((cast(size_t)baseAddr & (PAGESIZE - 1)) == 0);
    ...
    invariant
    {
        ...
        if (baseAddr)
        {
            ...
            assert_(baseAddr + npages * PAGESIZE == topAddr);
        }
...
void gc_enable()
{
    return locked!(void, () {
        assert_ (Invariant());
        scope (exit) assert_ (Invariant());
        assert_ (gc.disabled > 0);
        gc.disabled--;
    })();
}
